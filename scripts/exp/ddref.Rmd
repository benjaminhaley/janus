...
**Analysis**
  ...
  - [10B3 with metaregression](#10B3-meta) - Reproduce dose response figure applying the principals of meta-regression and postive coefficients.
  ...
  - [10B3 metareression on all data](#10B3-meta-all) Apply meta regression with positive coefficients to all of the datasets.
  - [10B4 metaregression on all data](#10B4-meta-all) Apply meta regression with positive coefficients to all of the datasets to generate profiles.
...
** New anlaysis **
  - [Data Funnel 2](#data_funnel_2) - Which data will we analyze?
  - [Data Cleaning 2](#cleaning_2) - Damn, data, you look good!
  - [Visualize 2](#visual_concordance_2) - Show what the data looks like.
...
count(d)
report <- update_report(report, d, "Without major contradictions in the primary literature")
...
fits <- get_likelihoods(data, model_10B2)

ggplot(fits, aes(o, l)) + geom_path()
...
m <- lq_model(data[data$modeled_in_10B3,])
...
This figure does not include male mice or BALB/c females from [Storer 1979][storer 1979].  Its not clear if these are excluded from the profile likelihood generation as well, or not.  In any case they fit the inclusion criteria, and should have been addressed.
...
likelihoods <- get_likelihoods(aggregate, fixed_o_model)
...
most_likely_o_in_new_data <- confidence_interval(tenb4$o, tenb4$new_lifespan)['middle']
p <- significance(most_likely_o_in_new_data, tenb4$o, tenb4$lifespan)
```

#### Result: How significant is the difference?
The new data suggests a most likely theta value of `r most_likely_o_in_new_data`.  The likelihood curve developed from the original data assigns this a p value < `r p`!
...
#### Figure: Combined animal likelihood profiles when using all animal data
Analagous to all of figure 10B4, we see the effect of using this new lifespan profile estimate on the ultimate profile likelihood.  Notably, the mean has shifted lower and now contains a 'double hump' a questionable shape that implies that certain intermediate values of ddref are less likely than values of either side of them!
...
animal <- tenb4 %>% 
  mutate(new_Radiobiological_prior=new_mean) %>%
...
. <- confidence_interval(o, tenb4[['lifespan']], 0.05)      # was   1.4 ( 0.5, 6.0)
. <- confidence_interval(o, tenb4[['new_lifespan']], 0.05)  # is   -0.1 (-0.2, 0.1)

# How did this affect all animal data?
. <- confidence_interval(o, tenthree[['Radiobiological_prior']], 0.05)      # was   0.4 ( 0.1, 3.5)
. <- confidence_interval(o, tenthree[['new_Radiobiological_prior']], 0.05)  # is   -0.1 (-0.2, 0.7)

# How did it affect the overall estimate
. <- confidence_interval(o, tenthree[['Combined_posterior']], 0.05)      # was   0.4 ( 0.1, 1.2)
. <- confidence_interval(o, tenthree[['new_Combined_posterior']], 0.05)  # is   -0.1 (-0.2, 0.9)
```
...
```{r}
. <- confidence_interval(beir_r$o, beir_r$l, 0.05)
. <- confidence_interval(my_r$o, my_r$l, 0.05)
```
...
```{r}
. <- confidence_interval(tenb4$o, tenb4$lifespan, 0.05)       # 1.4 ( 0.5, 6)
. <- confidence_interval(likelihoods$o, likelihoods$l, 0.05)  # 0.3 (-0.2, 6)
```
...
TODO: show meta regression when DDREF is constrained to a fixed value (after finding
      the appropriate value below

TODO: show how fits look on all data when DDREF is constrained to a fixed value
      can we actually fit the curves?
...
summarize_treatments <- function(df){
  summary <- ddply(df, .(dose, dose_rate, fractions), function(df2){ with(df2, c(
    "♀"=sum(sex=="♀"),
    "♂"=sum(sex=="♂")
  ))})
  ...
  table(summary, print.results=FALSE)
}
...
summary <- ddply(d, .(lab, strain, species), function(df){ with(df, c(
  treatments=summarize_treatments(df),
  references=references[[only(lab)]]
))})

table(summary, sanitize.text.function=function(x){x})
...
### Introduction:
A more complete dataset where I don't depend on the BEIR VII 
analysis cutoffs.
...
Clean
========================================================
Data has been filtered.  Now its time to prettify it.
...
# Save Data for later use
saveRDS(d, 'data/protraction.rds')
write.csv0(d,file='data/protraction.csv')
```
...
# Data
setwd('~/janus')
d <- readRDS('data/protraction.rds')
...
ggsave_for_ppt("/Users/benjaminhaley/Dropbox/Public/thesis-figures/lifespan-density-2.png")
```
...
```{r}
magnitude <- function(x) signif(x, digits=1)

d <- d %>%
  mutate(dose=magnitude(dose),
         dose_rate=magnitude(dose_rate),
         fractions=magnitude(fractions)) %>%
  group_by(cluster, dose, dose_rate, fractions) %>%
  arrange(lifespan) %>%
  mutate(survival=rank(-lifespan) / length(lifespan))
...
ggplot(d,
       aes(lifespan,
           survival,
           color=magnitude(dose),
           size=magnitude(dose_rate + 0.1),
           group=factor(paste(magnitude(dose), magnitude(dose_rate), magnitude(fractions))))) +
  geom_path() +
  facet_wrap(~ cluster, scales="free_x") +
  scale_color_continuous(
    guide = guide_legend(title = "dose (Gy)"),
  ...
  scale_size_continuous(
    guide = guide_legend(title = "dose rate (Gy/min)"),
    trans = "sqrt"
  ) +
  geom_vline(
    aes(xintercept=intended_assignment_age),
...
ggsave_for_ppt("/Users/benjaminhaley/Dropbox/Public/thesis-figures/survival-2.png")
...
# Median lifespans
median <- d %>%
  summarize(lifespan=median(lifespan))

ggplot(median, 
        aes(dose,
            lifespan,
            color=magnitude(dose_rate + 1e-6))) +
  geom_point() +
...
#### Figure: Median lifespans
Astoundingly linear results.  Strongly suggests that DDREF is 1 or so.
...
To compare effect sizes between studies which measure different outcomes (lifeshortening in humans, life shortening in mice, cancer rates in mice) we need a common outcome.

What happens if we apply cohen's D to to the data?  Is it reasonable?
...
cohens <- d %>%
  ...
  ungroup() %>%
  group_by(cluster) %>%
  mutate(cluster_n = length(cluster),
         cluster_s = sd(lifespan),
         cluster_u = mean(lifespan),
         pooled_s = )
