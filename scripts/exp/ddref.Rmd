...
- Analysis
  ...
  - [Reproduce BEIR 10-2](#10-2) - Reproduce dose response for atomic bomb survivors.
...
to_predict$risk <- exp(predict(model, newdata=to_predict))

aggregate <- to_predict %.%
  group_by(dose) %.%
  summarize(risk=mean(risk))

aggregate$err <- with(aggregate, risk / min(risk) - 1)

ggplot(aggregate, aes(dose, err)) + geom_point()


# TODO: Note that the sexes in lss behave quite differently, males are responding stronger.
...
[10B4-image]: http://dl.dropbox.com/u/1131693/bloodrop/10-2.jpg


##### Reproduce 10-2
This is my reproduction of 10-2 to prove that I am faithfully applying their methodology.
```{r}
```
