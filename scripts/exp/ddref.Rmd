...
scales <- scales %>%
  ungroup()  %>%
  mutate(max = min + max(scales$range),
...
analyze <- function(name, data, model, predict_fn) {
  
  intervals <- read.csv('data/theta_estimates.csv', as.is=TRUE)

  # Find likelihood profile of theta, o
  likelihoods <- get_likelihoods(data, model)
...
d <- d[!(d$dose > threshold_dose),]
count(d)
report <- update_report(report, d, paste("Dose below", 
                                         threshold_dose,
                                         "Sv threshold"))
...
# How many duplicates after filtering?
table0(d$duplicates)  # 33991 FALSE 9497 TRUE

count(d)
#    studies   clusters treatments    animals not vetted to exclude 
#         21         22        151      43488      10252          0 
...
# Save Data for later use
saveRDS(d, 'data/ddref_2.rds')
write.csv0(d,file='data/ddref_2.csv')
```
...
# Data
setwd('~/janus')
d <- readRDS('data/protraction_2.rds')
...
___________________________________________________________________
<a name="1/lifespan with high dose limits"></a>

1/lifespan with high dose limits
========================================================
*Last update: October 2014*

Show graphs like Storer 1979 10-B3 and 10-B4 but for this larger dataset.
...
# Data
data <- readRDS('data/ddref_2.rds')

# Mean Lifespans
# TODO: fix more than one dose in group 1003-21-3
aggregate = data %>%
  group_by(cluster, group_id, factor(dose), factor(dose_rate)) %>%
  summarize(age=mean(lifespan),
            n=length(lifespan),
            sd=sd(lifespan)/n^0.5,
            dose=only(dose),
            fractions=only(fractions),
            type=only(type),
            dose_rate=only(dose_rate))

# Save aggregated data for later use
# TODO fix write error 'index out of range'
# write.csv0(aggregate, file='data/10B4_all_data_aggregated_2.csv')

# aggregate %>% select(cluster, age, group_id) %>% arrange(age) %>% filter(get_cluster_number(cluster) == 1, age < 900)


# Remove problem data
# TODO migrage this up the chain
aggregate <- aggregate %>%
  filter(!group_id %in% c('1003-21-3', '1003-25-8', '9-4-24', '9-4-25', '9-4-29', '9-4-30', '9-4-31'))

aggregate <- rbind(
  aggregate, 
  aggregate %>% filter(dose == 0) %>% mutate(type = "C"))
...
ggsave_for_ppt("/Users/benjaminhaley/Dropbox/Public/thesis-figures/lifeshortening-2.png")
```

#### Results
Very linear, chronic seema little smaller.
...
What if we focus on the direct comparisons?
...
aggregate$both <- FALSE
for(c in unique(aggregate$cluster)){
  df <- aggregate %>% filter(cluster == c)
  if ('A' %in% df$type & 'C' %in% df$type){
    aggregate[aggregate$cluster == c,'both'] <- TRUE
    }
}
aggregate <- aggregate %>% filter(both)
...
chronic <- aggregate[aggregate$type == 'C' &
                     aggregate$dose > 0,]
...
ggplot(aggregate, aes(dose,
  ...
  geom_text(aes(label=fractions), alpha=0.5) +
...
ggsave_for_ppt("/Users/benjaminhaley/Dropbox/Public/thesis-figures/lifeshortening-2-comparisons-only.png")
```
