...
```{r}
# See how this affects downstream likelihood curves
setwd('~/janus')
...
# Merge with new data
tenthree <- merge(tenb4 %>% mutate(o=round(o, 2)),
...
@here
# Model
m <- model_meta_fixed_o(aggregate)
tau2 <- m$tau2

# Make predictions
to_predict <- get_data_to_predict(clusters=aggregate$cluster)
to_predict$p_my_analysis <- predict_meta(m, newdata=to_predict, clustered=TRUE)
...
ggplot(aggregate, aes(dose,
  ...
  geom_smooth(
    data=to_predict,
    aes(dose, p_my_analysis, linetype=type),
    method='lm',
    formula='y ~ x + I(x^2)',
    se=FALSE,
    color='red'
