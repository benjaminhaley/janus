...
```{r}
most_likely_o_in_new_data <- confidence_interval(likelihoods$o, likelihoods$l, 0.05)['middle'] 
p <- significance(most_likely_o_in_new_data, tenb4$o, tenb4$lifespan)  # 2e-13
```
...
animal <- tenb4 %>% 
  mutate(new_Radiobiological_prior=mean) %>%
...
summarize_treatments <- function(df){
  ...
  html_table(summary, print.results=FALSE)
}
...
html_table(summary, sanitize.text.function=function(x){x})
