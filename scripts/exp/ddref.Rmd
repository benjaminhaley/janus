...
**Analysis**
  ...
  - [10B3 and 10B4 on all data](#10B3-and-10B4-all-data) - Fit 1/lifespan models on all of the data.
...
<a name="10B3-and-10B4-all-data"></a>
...
my_r <- get_likelihoods(data %>% filter(modeled_in_10B3), model_meta)
...
# TODO: factor our these model definitions into a single place
model_10B3 <- function(data){
  glm(
    I(1/age - 1/control_age) ~ 
        dose * cluster
      + I(dose^2 / (fractions)) * cluster
      - cluster
      - 1,
    data=data,
    weights=1/sd^2
...
model_meta <- function(data){
  ...
  rma(
    yi,
    vi,
    mods = ~ a*cluster + B*cluster -a -B -cluster -1,
    data = data,
    method='ML'
  )
}
...
# TODO: factor out prediction interval code
# Define Prediction Intervals
to_predict <- expand.grid(
    fractions = c(1, 1000),
    dose = seq(0, 1.5, 0.1),
    cluster = unique(aggregate$cluster)
)
to_predict$type <- 'A'
to_predict$type[to_predict$fractions > 1] <- 'C'
to_predict <- to_predict %>%
  mutate(a = dose,
         B = dose^2 / (fractions))
control_ages <- unique(aggregate %>% select(control_age, cluster))
to_predict <- merge(to_predict, control_ages)

newmods <- model.matrix(~ a*cluster + B*cluster -a -B -cluster -1,
                        data=to_predict)

# Make predictions
to_predict$p_my_analysis <- predict(m, newmods=newmods)$pred
to_predict$p_10B3 <- predict(model_10B3(aggregate), newdata=to_predict)

# Make sure factor levels are correct
to_predict <- to_predict %>% mutate(cluster = order_levels_by_number(cluster))
...
ggplot(aggregate, aes(dose,
  ...
  geom_smooth(
    data=to_predict,
    aes(dose, p_10B3 + 1/control_age, linetype=type),
    method='lm',
    formula='y ~ x + I(x^2)',
    se=FALSE,
    color='black'
  ...
  geom_smooth(
    data=to_predict,
    aes(dose, p_my_analysis + 1/control_age, linetype=type),
    method='lm',
    formula='y ~ x + I(x^2)',
    se=FALSE,
    color='red'
...
# Model
regular <- get_likelihoods(aggregate, model_10B4) %>%
  mutate(likelihood_regular=l) %>%
  select(-l)
meta <- get_likelihoods(aggregate, model_meta) %>%
  mutate(likelihood_meta=l) %>%
  select(-l)
likelihoods <- merge(regular, meta)

# Show
ggplot(likelihoods) +
    geom_path(aes(o, likelihood_regular), color='black') +
    geom_path(aes(o, likelihood_meta), color='red')


# TODO: Double check the above, clean up the chart, it should actually look more like 10B4
