...
- Analysis
  ...
  - [Reproduce BEIR 10-3](#10-3) - Reproduce DDREF likelihood profiles for atomic bomb survivors.
...
# Data
setwd('~/janus')
source('scripts/util/util.R') # https://github.com/benjaminhaley/janus/blob/master/scripts/util/util.R
...
model_10B3 <- function(data){
    glm(
        I(1/age - 1/635.2) ~ dose + I(dose^2 / (fractions)) - 1,
        data=data,
        weights=1/sd^2
    )
}   
...
predictions$p_10B3 <- predict(m, newdata=predictions) + 1/635.2

# My Model
s <- data$modeled_in_10B3 & 
     data$in_my_analysis
m <- model_10B3(data[s,])
predictions$p_my_analysis <- predict(m, newdata=predictions) + 1/635.2
...
4. The regression intercept was [set to the control condition][wo-intercept]
...
library(dplyr)
source('scripts/util/util.R') 
# https://github.com/benjaminhaley/janus/blob/master/scripts/util/util.R
...
model_10B4 <- function(data, o){    
    glm(
        I(1/age - 1/635.2) ~ I(dose + o*dose^2 / (fractions)) - 1,
        data=data
    )
}

new_r <- get_likelihoods(o_range, data[data$modeled_in_10B4,])

ggplot(NULL) + 
  geom_path(aes(beir_r$o, beir_r$l)) +
  geom_path(aes(new_r$o, new_r$l), color='red') +
  ylim(0, 1) +
  
# Attempt 1: Expand data set
# Tried to make one row for every mouse and run the regression
# this did not work.
# https://www.dropbox.com/s/jk5ie68nzhbgyux/Screenshot%202014-05-21%2012.39.52.png
# Attempt 2: Fixed effect
# Fail, though oddly different
...
confidence_interval(beir_r$o + 1, beir_r$mean_likelihood)
# should be "1.5 (1.0, 4.4)"
...
```{r}
ggplot(beir_r, aes(o)) + 
    geom_path(aes(y=lifespan_likelihood), linetype='dashed') + 
    geom_path(aes(y=tumor_likelihood), linetype='dotted') + 
    geom_path(aes(y=mean_likelihood)) + 
...
Reproduce BEIR 10-3 (from a reproduction of 10-2)
========================================================
*Last update: May 2014*
...
weighted_fit <- function(...) geom_smooth(
  ...,
  method='lm', 
  aes(weight=1/sd),
  se=FALSE,
...
ggplot(data, aes(dose, err)) +
  geom_pointrange(aes(ymax=err + 2*sd, ymin=err - 2*sd)) +
  weighted_fit(formula='y ~ x - 1') +
  weighted_fit(formula='y ~ I(x + 0.3 * x^2) - 1', 
               linetype='dotted') +
  weighted_fit(formula='y ~ I(x + 0.7 * x^2) - 1',
               linetype='dashed') + 
...
```
#### 10-2 with fit
These fits to the data are not perfect, but a pretty close reproduction.  Close enough that I have hope that they may have fit things in this way and that we can develop the profile likelihood values by this technique.

If they did, this is strange.  They seem to be fitting a linear quadratic equation to data from a poisson regression which is e^(linear quadratic).  I don't know what to make of this.
