...
**Analysis**
  ...
  - [10B3 metareression on all lifespan data](#10B3-meta-all) Apply meta regression with positive coefficients to all of the lifespan datasets.
  - [10B4 metaregression on all lifespan data](#10B4-meta-all) Apply meta regression with positive coefficients to all of the lifespan datasets to generate profiles.
...
```{r}
. <- confidence_interval(beir_r$o, beir_r$l, 0.05)  # "1.4 (0.5, 6)"
. <- confidence_interval(my_r$o, my_r$l, 0.05)      # "1.4 (0.3, 6)"
```
...
write.csv0(likelihoods, 'data/10B2_meta_fits.csv')
...
# Data
data <- read.csv('data/edwards_1992.csv', sep='\t')
data_10B2_likelihoods <- read.csv('data/10B2_fits.csv')
...
setwd('~/janus')
write.csv0(likelihoods, 'data/10B2_meta_likelihoods.csv')
```
...
likelihoods <- get_likelihoods(data, model_meta_10_3, o_range = seq(-2, 6, by=0.01))
# TODO: fix vi in previous model
# @here
...
#### BEIR 10-3 meta
We become even more confident that the atomic bomb survivor data is only very slightly curved.
