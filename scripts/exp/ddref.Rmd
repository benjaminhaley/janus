...
# and they were messing with my later analysis
...
ggplot(g,
       aes(lifespan,
           ...
           y=..scaled..)) +
  geom_density(adjust=2) +
  facet_grid(dose~cluster, scales='free_x') +
  scale_colour_gradient(
    guide = guide_legend(title = "Dose (Gy)"),
...
tenb4 <- merge(tenb4 %>% mutate(o=round(o, 2)),
               likelihoods %>% mutate(o=round(o, 2))) %>%
  mutate(new_lifespan=l,
         new_mean=(tumor + new_lifespan) / 2,
         new_tumor=tumor) %>%
  select(-l)
...
ggplot(g %>% filter(source == 'lifespan'), 
       aes(o, likelihood, color=new, alpha=new)) +
  geom_path() +
...
# Helpers
beir_r <- get_likelihoods(data %>% filter(modeled_in_10B3), fixed_o_model)
my_r <- get_likelihoods(data %>% filter(modeled_in_10B3), model_meta_fixed_o)
...
TODO: this should model the previous analysis, finding the profile then fitting
TODO: the graph should look like the previous too.
...
likelihoods <- get_likelihoods(aggregate, model_meta_fixed_o)
...
# Fit with best theta value
m <- model_meta_fixed_o(aggregate, o)
...
to_predict$p_my_analysis <- predict_meta_fixed_o(m, newdata=to_predict, clustered=TRUE)
...
Analagous to figure ??? from BEIR VII we see ordinary linear quadratic models (black) fit to 1/lifespan of accutely, A, and chronically, C, mice at various doses in Gy.  Straight lines correspond to chronic predictions while curved lines correspond to accute predictions.  Grey error bards show measured uncertainty, red error bars show measured uncertainty plus estimated heterogeneity by meta-regression.  The best fit after meta regression across all clusters is shown in red dotted lines.

This figure does not change much from before, expect that it reveals a high level of unmeasured uncertainty that is revealed in some data points (e.g. cluster 8).  The result is more tollerance for curvature values because the data is not constrained to fit a very tight model.


```{r}
# Show 10B4 with all data meta
ggplot(likelihoods, aes(o, l)) + 
  geom_path() + 
  # TODO: make sure cood_cartisian is used in place of xlim/ylim and used consistently
  coord_cartesian(ylim=c(-0.1,1.5)) +
  # TODO: be sure we are using theta, o, consistently.  It should be θ
  xlab("θ (DDREF.lss - 1)") + 
  ylab("likelihood")
```

#### Figure: Likelihood profile when using meta analysis on all data
Analogous to the lifespan likelihood curve from figure 10B4 but using all of the animal data and applying meta-analysis, instead of limiting to that avaiable from storer 1979. Note that DDREF has been restored to something similar to the original value, far more reasonable.
