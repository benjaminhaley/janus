...
# Stray fixes
## Some of the beagles were missing gender
...
##### Duplicate controls
Control animals may control for multiple clusters.  For example the same
cohort could control for mice exposed to gamma rays at 114 days and x-rays
at 67 days.  Therefore control groups ought to be duplicated and included
in each cluster that they might control for.

Concretely a control groups should match sex, species, strain, and lab.  It
should be duplicated for each unique quality and age of first exposure provided
the aforementioned are the same.
...
d <- ddply(d, .(cluster), function(df) {
  ...
  ddply(treatment, .(quality, intended_age_of_treatment), function(treatment_group){
    
    # Add control to each treatment group
    df2 <- rbind(treatment_group, control)
    
    # Define quality and intended age of treatment by the treatment group
    # i.e. 
    #    NA, quality = 'none (control)'  ->  42 days, quality = 'gamma'
    df2$intended_age_of_treatment <- treatment_group$intended_age_of_treatment[1]
    df2$quality <- treatment_group$quality[1]
    
    # Add quality to the cluster name
    df2$cluster <- with(df2, paste(cluster, 
                                   intended_age_of_treatment, 
                                   quality,
                                   sep='--'))
...
table0(d$duplicates)  # 16160 TRUE, 18244 FALSE
...
for(c in unique(d$cluster)) {
  ...
  pretty_cluster_name = with(elements, paste(
    number_of_animals, sex, strain, pluralize(species), lab,
...
ggplot(d, 
       aes(lifespan, 
           color=dose,
           group=factor(paste(dose, dose_rate, fractions, protracted)),
           linetype=protracted)) + 
...
#TODO: Order this graph by:
#  Age at exposure
#  Instititution
#  Gender

# TODO: Look carefully at C57BL/Cnb controls from SCK/CEN
# TODO: Make smoothing adjustment proportional to group size

```

A few observations:
- A second hump in some examples, e.g. RFM/Un Female mice at ORNL
- 4 Groups less than 30 days old show hormetic responses
- Females seem to have stronger responses than males on average
...
d$treatment <- with(d, paste(round(dose, 2), 
                             'Gy in', 
                             round(fraction_time, 2), 
                             'min'))
d$treatment[d$dose == 0] <- 'control'
d$treatment[d$fractions > 1] <- paste(d$fractions,
                                      'fractions of',
                                      d$treatment, 
                                      )[d$fractions > 1]
g <- ggplot(d, 
...
direct.label(g, list("top.bumpup", cex=0.4))
```
