...
# TODO(ben) make sure we are never missing those factors used to cluster on
# TODO(ben) make sure the names of the radiation treatments are consistent, so that gamma is the same as gamma gamma, before clustering.
# TODO(ben) migrate any cleaning steps before the funnel into the cleanup
#           script
```
...
Furthermore, controls groups may control for multiple different age
at treatments.  Therefore they should be duplicated for each cluster
they belong in.
...
d <- ddply(d, .(cluster), function(df) {
  ...
  ddply(treatment, .(intended_age_of_treatment), function(treatment_group){
    
    # Add control to each treatment group
    df2 <- rbind(treatment_group, control)
    
    # Define intended age of treatment by the treatment group
    # i.e. 
    #    42 days, dose = 0  ->  67 days, dose = 0
    df2$intended_age_of_treatment <- treatment_group$intended_age_of_treatment[1]
    
    # Add quality to the cluster name
    df2$cluster <- with(df2, paste(cluster, intended_age_of_treatment, sep='--'))
    
    df2
  })
})

# TODO(ben) Label duplicated data and make a count of it.
```
...
for(c in unique(d$cluster)) {
  ...
  pretty_cluster_name = with(elements, paste(
    number_of_animals, sex, strain, pluralize(species),
